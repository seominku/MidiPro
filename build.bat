...
set INC=/I external\rtmidi /I external\rtaudio /I external\imgui /I external\imgui\backends /I external\vst3sdk /I src
set CFLAGS=/nologo /EHsc /std:c++17 /utf-8 /O2 /W3 /D__WINDOWS_MM__ /D__WINDOWS_WASAPI__ /D_SILENCE_ALL_CXX17_DEPRECATION_WARNINGS /D_CRT_SECURE_NO_WARNINGS /DRELEASE=1
...
set SEQ=src\sequencer\TimeBase.cpp src\sequencer\Track.cpp src\sequencer\Song.cpp src\sequencer\SmfFile.cpp src\sequencer\Player.cpp
set MIDI=src\midi\MidiMessage.cpp src\midi\RtMidiDevice.cpp external\rtmidi\RtMidi.cpp src\midi2\Ump.cpp
set AUDIO=src\audio\Synth.cpp src\audio\SynthPreset.cpp src\audio\RtAudioEngine.cpp external\rtaudio\RtAudio.cpp
...
set GUILIBS=winmm.lib ole32.lib oleaut32.lib shell32.lib advapi32.lib user32.lib d3d11.lib dxgi.lib d3dcompiler.lib comdlg32.lib dwmapi.lib

echo === Building GUI app (MidiPro.exe) ===
cl %CFLAGS% %INC% src\main_gui.cpp src\gui\App.cpp src\gui\Panels.cpp %MIDI% %AUDIO% %SEQ% %GUITAR% %MAPPING% %PROJECT% %VST% %IMGUI% %GUILIBS% /Fe:build\MidiPro.exe /Fo:build\ /link /SUBSYSTEM:WINDOWS
...
cl %CFLAGS% %INC% tests\test_sequencer.cpp %SEQ% %GUITAR% /Fe:build\MidiProSeqTests.exe /Fo:build\
if errorlevel 1 ( echo [ERROR] seq test build failed & exit /b 1 )
cl %CFLAGS% %INC% tests\test_synth.cpp src\audio\Synth.cpp src\audio\SynthPreset.cpp /Fe:build\MidiProSynthTests.exe /Fo:build\
...
cl %CFLAGS% %INC% tests\test_undo.cpp src\sequencer\Track.cpp src\sequencer\Song.cpp /Fe:build\MidiProUndoTests.exe /Fo:build\
...
cl %CFLAGS% %INC% tests\test_project.cpp src\project\Project.cpp src\audio\SynthPreset.cpp src\mapping\MidiMap.cpp src\sequencer\Track.cpp src\sequencer\Song.cpp /Fe:build\MidiProProjectTests.exe /Fo:build\
